...
class DateTimeAutoBinding extends HTExternalClass {
  ...
  dynamic memberGet(String varName, {String from = HTLexicon.global}) {
    switch (varName) {
      case 'DateTime':
        return ({positionalArgs, namedArgs, typeArgs}) => DateTime(
            positionalArgs[0],
            positionalArgs.length > 1 ? positionalArgs[1] : 1,
            positionalArgs.length > 2 ? positionalArgs[2] : 1,
            positionalArgs.length > 3 ? positionalArgs[3] : 0,
            positionalArgs.length > 4 ? positionalArgs[4] : 0,
            positionalArgs.length > 5 ? positionalArgs[5] : 0,
            positionalArgs.length > 6 ? positionalArgs[6] : 0,
            positionalArgs.length > 7 ? positionalArgs[7] : 0);
      case 'DateTime.utc':
        return ({positionalArgs, namedArgs, typeArgs}) => DateTime.utc(
            positionalArgs[0],
            positionalArgs.length > 1 ? positionalArgs[1] : 1,
            positionalArgs.length > 2 ? positionalArgs[2] : 1,
            positionalArgs.length > 3 ? positionalArgs[3] : 0,
...
extension DateTimeBinding on DateTime {
  dynamic htFetch(String varName) {
    switch (varName) {
      ...
      case 'toUtc':
        return ({positionalArgs, namedArgs, typeArgs}) => toUtc();
      case 'toString':
