...
class TableBorderAutoBinding extends HTExternalClass {
  ...
  dynamic memberGet(String varName, {String from = HTLexicon.global}) {
    switch (varName) {
      case 'TableBorder':
        return ({positionalArgs, namedArgs, typeArgs}) => TableBorder(
            ...
            horizontalInside:
                namedArgs.containsKey('horizontalInside') ? namedArgs['horizontalInside'] : BorderSide.none,
            verticalInside: namedArgs.containsKey('verticalInside') ? namedArgs['verticalInside'] : BorderSide.none);
      case 'TableBorder.all':
        return ({positionalArgs, namedArgs, typeArgs}) => TableBorder.all(
      ...
      case 'TableBorder.symmetric':
        return ({positionalArgs, namedArgs, typeArgs}) => TableBorder.symmetric(
            inside: namedArgs.containsKey('inside') ? namedArgs['inside'] : BorderSide.none,
            outside: namedArgs.containsKey('outside') ? namedArgs['outside'] : BorderSide.none);
      case 'TableBorder.lerp':
        return ({positionalArgs, namedArgs, typeArgs}) =>
            TableBorder.lerp(positionalArgs[0], positionalArgs[1], positionalArgs[2]);
      default:
...
extension TableBorderBinding on TableBorder {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('TableBorder');
      case 'top':
        return top;
      case 'right':
        return right;
