...
extension FlowParentDataBinding on FlowParentData {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('FlowParentData');
      case 'offset':
        return offset;
      case 'detach':
...
class RenderFlowAutoBinding extends HTExternalClass {
  ...
  dynamic memberGet(String varName, {String from = HTLexicon.global}) {
    switch (varName) {
      case 'RenderFlow':
        return ({positionalArgs, namedArgs, typeArgs}) => RenderFlow(
...
extension RenderFlowBinding on RenderFlow {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('RenderFlow');
      case 'parentData':
        return parentData;
      case 'debugCreator':
        return debugCreator;
      ...
      case 'getChildSize':
        return ({positionalArgs, namedArgs, typeArgs}) => getChildSize(positionalArgs[0]);
      case 'paintChild':
        return ({positionalArgs, namedArgs, typeArgs}) => paintChild(positionalArgs[0],
