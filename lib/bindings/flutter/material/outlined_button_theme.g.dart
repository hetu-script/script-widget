    ...
    switch (varName) {
      case 'OutlinedButtonThemeData':
        return ({positionalArgs, namedArgs, typeArgs}) =>
            OutlinedButtonThemeData(style: namedArgs.containsKey('style') ? namedArgs['style'] : null);
      case 'OutlinedButtonThemeData.lerp':
        return ({positionalArgs, namedArgs, typeArgs}) =>
            OutlinedButtonThemeData.lerp(positionalArgs[0], positionalArgs[1], positionalArgs[2]);
      default:
...
extension OutlinedButtonThemeDataBinding on OutlinedButtonThemeData {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('OutlinedButtonThemeData');
      case 'style':
        return style;
      case 'hashCode':
        return hashCode;
...
extension OutlinedButtonThemeBinding on OutlinedButtonTheme {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('OutlinedButtonTheme');
      case 'data':
        return data;
      case 'child':
        return child;
