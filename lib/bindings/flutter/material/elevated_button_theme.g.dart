    ...
    switch (varName) {
      case 'ElevatedButtonThemeData':
        return ({positionalArgs, namedArgs, typeArgs}) =>
            ElevatedButtonThemeData(style: namedArgs.containsKey('style') ? namedArgs['style'] : null);
      case 'ElevatedButtonThemeData.lerp':
        return ({positionalArgs, namedArgs, typeArgs}) =>
            ElevatedButtonThemeData.lerp(positionalArgs[0], positionalArgs[1], positionalArgs[2]);
      default:
...
extension ElevatedButtonThemeDataBinding on ElevatedButtonThemeData {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('ElevatedButtonThemeData');
      case 'style':
        return style;
      case 'hashCode':
        return hashCode;
...
extension ElevatedButtonThemeBinding on ElevatedButtonTheme {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('ElevatedButtonTheme');
      case 'data':
        return data;
      case 'child':
        return child;
