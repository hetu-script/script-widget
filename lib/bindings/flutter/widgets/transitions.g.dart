...
extension SlideTransitionBinding on SlideTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('SlideTransition');
      case 'textDirection':
...
extension ScaleTransitionBinding on ScaleTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('ScaleTransition');
      case 'alignment':
        return alignment;
      case 'child':
        return child;
...
extension RotationTransitionBinding on RotationTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('RotationTransition');
      case 'alignment':
        return alignment;
      case 'child':
        return child;
    ...
    switch (varName) {
      case 'SizeTransition':
        return ({positionalArgs, namedArgs, typeArgs}) => SizeTransition(
            key: namedArgs.containsKey('key') ? namedArgs['key'] : null,
            axis: namedArgs.containsKey('axis') ? namedArgs['axis'] : Axis.vertical,
            sizeFactor: namedArgs['sizeFactor'],
            axisAlignment: namedArgs.containsKey('axisAlignment') ? namedArgs['axisAlignment'] : 0.0,
            child: namedArgs.containsKey('child') ? namedArgs['child'] : null);
      default:
        throw HTErrorUndefined(varName);
    }
...
extension SizeTransitionBinding on SizeTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('SizeTransition');
      case 'axis':
        return axis;
      case 'axisAlignment':
...
extension FadeTransitionBinding on FadeTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('FadeTransition');
      case 'opacity':
        return opacity;
...
extension SliverFadeTransitionBinding on SliverFadeTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('SliverFadeTransition');
      case 'opacity':
        return opacity;
      case 'alwaysIncludeSemantics':
        return alwaysIncludeSemantics;
...
extension RelativeRectTweenBinding on RelativeRectTween {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('RelativeRectTween');
      case 'begin':
        return begin;
      case 'end':
        return end;
...
extension PositionedTransitionBinding on PositionedTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('PositionedTransition');
      case 'child':
        return child;
      case 'listenable':
        return listenable;
  ...
  dynamic memberGet(String varName, {String from = HTLexicon.global}) {
    switch (varName) {
      case 'RelativePositionedTransition':
        return ({positionalArgs, namedArgs, typeArgs}) => RelativePositionedTransition(
            key: namedArgs.containsKey('key') ? namedArgs['key'] : null,
            rect: namedArgs['rect'],
            size: namedArgs['size'],
            child: namedArgs['child']);
...
extension RelativePositionedTransitionBinding on RelativePositionedTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('RelativePositionedTransition');
      case 'size':
        return size;
      case 'child':
        return child;
  ...
  dynamic memberGet(String varName, {String from = HTLexicon.global}) {
    switch (varName) {
      case 'DecoratedBoxTransition':
        return ({positionalArgs, namedArgs, typeArgs}) => DecoratedBoxTransition(
            key: namedArgs.containsKey('key') ? namedArgs['key'] : null,
...
extension DecoratedBoxTransitionBinding on DecoratedBoxTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('DecoratedBoxTransition');
      case 'decoration':
        return decoration;
      case 'position':
        return position;
...
extension AlignTransitionBinding on AlignTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('AlignTransition');
      case 'widthFactor':
        return widthFactor;
      case 'heightFactor':
        return heightFactor;
...
extension DefaultTextStyleTransitionBinding on DefaultTextStyleTransition {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('DefaultTextStyleTransition');
      case 'textAlign':
        return textAlign;
      case 'softWrap':
        return softWrap;
...
extension AnimatedBuilderBinding on AnimatedBuilder {
  dynamic htFetch(String varName) {
    switch (varName) {
      case 'typeid':
        return const HTTypeId('AnimatedBuilder');
      case 'builder':
        return builder;
      case 'child':
        return child;
